...
export const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    const createNebula = () => {
      ...
      nebula.rotation.x = 0;
      threeRefs.current.nebula = nebula;
      threeRefs.current.scene.add(nebula);
    };
    ...
    const createAtmosphere = () => {
      ...
      threeRefs.current.scene.add(atmosphere);
    };
  ...
  useEffect(() => {
    ...
    // Set initial states to prevent flash (skip if ref is missing)
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const containerRectTop = container.getBoundingClientRect().top + window.scrollY;
      const containerHeight = container.offsetHeight;
      const windowHeight = window.innerHeight;
      const maxScroll = Math.max(containerHeight - windowHeight, 1);
      const localScrollY = Math.min(Math.max(window.scrollY - containerRectTop, 0), maxScroll);
      const progress = Math.min(Math.max(localScrollY / maxScroll, 0), 1);
      ...
      refs.mountains.forEach((mountain, i) => {
        const speed = 1 + i * 0.9;
        const targetZ = mountain.userData.baseZ + localScrollY * speed * 0.5;
        if (refs.nebula) {
